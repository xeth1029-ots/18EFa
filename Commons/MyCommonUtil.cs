...
{
    /// <summary>
    /// 放置 Turbo.MVC.Base3 專用的 CommonUtil 擴充,
    ...
    public class MyCommonUtil : Turbo.Commons.CommonUtil
    {
        ...
        /// <returns></returns>
        public static int GetPageRowIndex(int pageSize, int pageIdx, int rowIndex)
        {
            int rtn = 0;
            //int pagesize = model.PagingInfo.PageSize;
            //int pageidx = model.PagingInfo.PageIdx;
            rtn = (pageSize * (pageIdx - 1) + rowIndex + 1);
            return rtn;
        }
        ...
        public static DateTime? TransTwToDateTime(string twDate)
        {
            ...
            {
                Exception ex = new Exception("無法將民國日期字串轉換成 DateTime? 型別值。");
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static T ToEnum<T>(string id)
        {
            T action = default(T);
            try
            {
                action = (T)Enum.Parse(typeof(T), id);
            }
            catch (Exception ex)
            {
                LOG.Error(ex.Message, ex);
        ...
        public static string ComputeHash(string src)
        {
        ...
        public static string GetIpAddress(HttpContext context)
        {
            ...
            string cst_HTTP_X_FORWARDED_FOR = "HTTP_X_FORWARDED_FOR";
            string cst_REMOTE_ADDR = "REMOTE_ADDR";
            //Look for a proxy address first
            str_UserHostIp = context.Request.ServerVariables[cst_HTTP_X_FORWARDED_FOR];
            //If there is no proxy, get the standard remote address
            if (string.IsNullOrEmpty(str_UserHostIp) || str_UserHostIp.Equals("unknown", StringComparison.OrdinalIgnoreCase))
            {
                str_UserHostIp = context.Request.ServerVariables[cst_REMOTE_ADDR];
            }
            if (string.IsNullOrEmpty(str_UserHostIp) || str_UserHostIp.Equals("unknown", StringComparison.OrdinalIgnoreCase))
            {
                str_UserHostIp = context.Request.UserHostAddress;
            }
        ...
        /// <returns></returns>
        public static string GetMASKIDNO1(string pid)
        {
            string pid_mk = "*****";
            if (string.IsNullOrEmpty(pid)) return pid_mk;
            if (pid.Length > 4)
            {
                pid_mk = string.Concat(pid.Substring(0, 2), "******", pid.Substring(pid.Length - 2, 2));
                return pid_mk;
            }
            if (pid.Length > 1)
            {
                pid_mk = string.Concat(pid.Substring(0, 1), "******");
                return pid_mk;
            }
            return pid_mk;
        }
