...
namespace WDACC.Commons
{
    public class FileTypeDetector
    {
        ...
        private static readonly Dictionary<byte[], string> _fileSignatures = new Dictionary<byte[], string>
        {
            ...
            { new byte[] { 0x50, 0x4B, 0x03, 0x04 }, "zip" } // Also used by DOCX, XLSX, PPTX,odt,odp,ods
        };
        ...
        public static bool IsFileTypeValid(HttpPostedFileBase myPostedFile, string filePath)
        {
            ...
            // ".xls,.xlsx,.doc,.docx,.ppt,.pptx,.pdf,.jpg,.png,.msg,.odt,.ods";
            if (s_ReadlFileType == "zip")
            {
                string[] extfiles = { "zip", "docx", "xlsx", "pptx", "odt", "ods" };
                return extfiles.Contains(fileExtension);
            }
            else if (s_ReadlFileType == "jpg" || s_ReadlFileType == "png")
            {
                string[] extfiles = { "png", "jpg", "jpeg" };
                return extfiles.Contains(fileExtension);
            }
            else if (s_ReadlFileType == "doc")
            {
                string[] extfiles = { "doc", "xls", "msg", "ppt" };
                return extfiles.Contains(fileExtension);
            }
            else if (s_ReadlFileType == "pdf" && fileExtension == "pdf")
            {
                return true;
            }
            return false; //return s_ReadlFileType == fileExtension; //pdf=pdf
        }
